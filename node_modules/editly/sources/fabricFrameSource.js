...
async function imageFrameSource({ verbose, params, width, height }) {
  ...
  const smartPath = params.path.includes('http') ? params.path : fileUrl(params.path);
  const imgData = await new Promise((resolve) => fabric.util.loadImage(smartPath, resolve));
